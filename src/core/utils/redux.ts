...
export function createObjectReducer<T>(name: string, parentKey?: string, listKey?: string): T {
  ...
  if (parentKey) {
    result[`${name}`] = (state: any, action: any) => {
      if (listKey) {
        const dataInList = state.get(listKey).get('data').filter((item: any) => item.get('id') === action.payload.id);
...
export function createArrayReducer<T>(name: string, parentKey?: string): T {
  ...
  if (parentKey) {
    ...
    result[`${name}Success`] = (state: any, action: any) => {
      ...
      const data = currentPage === 1 ? fromJS(dataGet) : state.getIn([parentKey, 'data']).concat(fromJS(dataGet).filter((item: any) => state.getIn([parentKey, 'data']).indexOf(item) < 0));
      return (state
        .setIn([parentKey, 'loading'], false)
    ...
    result[`${name}Success`] = (state: any, action: any) => {
      ...
      const currentPage = action.payload.metadata.page;
      const dataGet = action.payload.results;
      const data = currentPage === 1 ? fromJS(dataGet) : state.get('data').concat(fromJS(dataGet).filter((item: any) => state.get('data').indexOf(item) < 0));
      return (state
        .set('loading', false)
        .set('data', data)
